...
PRIVATE int chosen_algorithm = 0; /* User defined */
 ...
 */
  ...
  register struct hole *hp, *previous_ptr, *biggest_hole, *previous_biggest_hole;
  ...
  if (chosen_algorithm) {
    do {
      ...
      if (hp != NIL_HOLE) {
        biggest_hole = hp;
        previous_biggest_hole = NIL_HOLE;
      } else {
        return NO_MEM;
      }

      while (hp != NIL_HOLE && hp->h_base < swap_base) {
        if (hp->h_len > biggest_hole->h_len) {
          biggest_hole = hp;
          previous_biggest_hole = previous_ptr;
        }
        previous_ptr = hp;
        hp = hp->h_next;
      }

      if (biggest_hole->h_len >= clicks) {
        old_base = biggest_hole->h_base;
        biggest_hole->h_base += clicks;
        biggest_hole->h_len -= clicks;
        if (biggest_hole->h_len == 0 && previous_biggest_hole != NIL_HOLE)
          del_slot(previous_biggest_hole, biggest_hole);
        return old_base;
      }
    } while (swap_out());
  } else {
 ...
 *				do_hole_map				     *
 *===========================================================================*/
int do_hole_map(void) {
  phys_clicks buffer[NR_HOLES*2+1];
  struct hole *hp;
  int capacity = mm_in.m1_i1 / sizeof(phys_clicks)*2, i = 0;
  int user_pid = mm_in.m_source, n_bytes = mm_in.m1_i1;
  char* user_buffer = mm_in.m1_p1;

  hp = hole_head;
  while (i < capacity && hp!= NIL_HOLE) {
    buffer[i] = hp->h_len;
    buffer[++i] = hp->h_base;
    hp = hp->h_next;
    i++;
  ...
  sys_copy(MM_PROC_NR, D, (phys_bytes)buffer, user_pid, D, (phys_bytes)user_buffer, n_bytes);
  return (i-1)/2;
}
 ...
 *				do_worst_fit				     *
 *===========================================================================*/
int do_worst_fit(void) {
  if (mm_in.m1_i1 == 0 || mm_in.m1_i1 == 1)
    chosen_algorithm = mm_in.m1_i1;
  return 0;
}
